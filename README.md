...
The screenshot refinement uses locally hosted Bai Jamjuree fonts, staggered hero feature cards, decorative image masks, button-controlled stacked service cards, two project cards, three team profiles, an industrial offer banner, photo-led testimonials, and a layered footer. The contact section embeds `[fluentform id="1"]`. Install and activate Fluent Forms, and create/preserve form 1 on the target site. Edit fields, service options, submit text, and confirmation under **Fluent Forms → Forms**; view submissions under **Fluent Forms → Entries**. Notifications require working email delivery. The theme ZIP does not contain the plugin or its form data. If form 1 cannot render, the section shows a direct email link instead. The theme does not intercept Fluent Forms submissions. Footer subscriptions link to the WordPress RSS feed.
